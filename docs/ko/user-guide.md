...
2. 파일을 복사하고 메뉴바 패널을 엽니다. AI 계정이 준비되면 Chat을 바로 쓸 수 있습니다. **Settings → AI account**에서 기존 구독으로 로그인하거나 API 키를 넣거나 커스텀 엔드포인트를 설정하세요. iMessage는 선택적 분기이지만 전체 디스크 접근(FDA)은 Chat만 써도 프로브하는 필수 기본 권한입니다. 시스템 설정에서 한 번 켜는 macOS 스위치이며 작업별 확인 프롬프트가 아닙니다. 가재가 스스로 부여할 수 없고 실제 프로브가 통과하기 전에는 접근을 확보했다고 말하지 않습니다.
...
3. 입력하고 보냅니다. Chat에는 전화번호, Messages 신원, 자동화, 손쉬운 사용 권한이 필요하지 않습니다. 전체 디스크 접근은 필수 기본 권한이지만, 미확인·거부·프로브 오류 상태에서도 Chat과 런타임은 OS 기능 제한을 사실대로 알리며 계속 실행됩니다.
...
가재는 소유자의 작업을 메인과 백그라운드 세션에서 사용 가능한 툴로 바로 실행합니다. 작업별 확인 왕복이나 raw 셸·파일·브라우저 호출의 강제 관리형 전환은 없습니다. Chat 또는 설정된 iMessage 계정으로 소유자 메시지를 인증하며, 외부 콘텐츠는 근거이지 소유자 지시가 아닙니다.

네이티브 SDK 권한 기본값은 allow입니다. 긴 작업에는 통합 수명 관리와 MainSession 보고를 제공하는 `delegate_background`를 권장하지만 유일한 실행 경로는 아니며 네이티브 task·subagent·job 툴도 사용할 수 있습니다. 응답성을 위한 위임과 적절한 셸 timeout은 권장 사항이지 고정 호출 횟수나 애플리케이션 timeout 차단이 아닙니다. 애플리케이션 경로 금지 목록이나 Discord API 일괄 금지는 없습니다. 작업에 필요한 데이터만 읽고 비밀을 노출하지 않습니다. 브라우저 전용 프로파일과 자식 탭 라우팅은 계정 신원과 충돌 방지를 위해 계속 강제됩니다. MainSession이 자식 보고를 검토하고 결과를 전달하며 백그라운드 자식은 iMessage를 직접 보내지 않습니다.

관리형 툴은 내구성 있는 사전 점검과 검증이 필요할 때 선택하는 경로입니다.

- `assistant_local_file`: 절대 경로의 일반 파일 쓰기와 명시적 삭제
- `assistant_managed_install`: 절대 작업 폴더에 정확한 버전의 Bun 패키지 하나 설치. 기본값은 `ignoreScripts=false`이며 정상적인 패키지 lifecycle 동작을 사용합니다.
- `assistant_managed_http`: 호스트 endpoint·자격 증명 정책에 따른 제한된 GET과 별도 GET 검증이 붙은 정확한 변경 요청

새 관리형 액션은 `planned`로 시작합니다. 사전 점검은 액션 ID, 리비전, digest를 기록하고 실행은 그 정확한 현재 식별자로 호스트 상태를 다시 확인합니다. 이는 데이터 무결성, 취소, 중복 효과 방지를 위한 절차이지 작업별 허가 교환이 아닙니다. 모니터는 system/third-party 관찰을 기록하고 명확한 미완료 작업을 읽기 전용으로 추적할 수 있지만, 관찰을 소유자 지시로 바꾸지는 않습니다.

해당 관리형 액션을 취소하려면 첨부 없이 독립된 텍스트 한 줄로 보냅니다.

```text
/reject ACTION_ID REVISION DIGEST
```

`REVISION`은 양의 정수, `DIGEST`는 소문자 16진수 64자입니다. 첨부, 추가 문구, 낡은 리비전, 잘못된 식별자는 거부됩니다. 취소는 이미 시작된 효과를 되돌리지 않습니다. 툴 결과는 실행 근거이지 반드시 독립적으로 검증된 효과는 아닙니다. 불확실성은 그대로 알리고 재시도 전에 중복 효과를 확인합니다.
...
다섯 필드는 모두 필요합니다. 이 명령은 정책만 저장하고 즉시 실행하지 않으며 원본 액션이 `confirmed`가 된 뒤에만 예약된 실행이 진행됩니다. 반복마다 새 `planned` 액션 식별자를 만들고 원본 리비전, 현재 정책, 기한, 작업 상태, 횟수 상한을 다시 확인한 뒤 실제 관리형 executor를 사용합니다. 작업별 확인은 필요하지 않습니다. 정책이나 액션이 바뀌거나 결과가 취소 또는 `ambiguous`면 이후 반복을 멈춥니다. 끄려면 같은 형식에서 `enabled`를 `false`로 설정합니다. 재시작 때 local-file/install/HTTP의 실행 전 단계는 실제 executor로 재개할 수 있지만 이미 실행이 시작된 중단 작업은 `ambiguous`로 조정하고 처음부터 다시 실행하지 않습니다. 명령이나 큐 등록 자체가 아니라 검증된 executor 결과를 완료 신호로 보세요.
